...
export class SourceHintEngine {
     ...
     */
    generateHints(element, componentInfo, adapter) {
        const hints = [];
        let maxConfidence = 0;
     ...
     */
    extractStyleHint(element) {
        // Check for scoped style attribute (Vue)
        const scopedAttr = Array.from(element.attributes).find((attr) => attr.name.startsWith('data-v-'));
        if (scopedAttr) {
            return {
                kind: 'style-file',
     ...
     */
    filterByConfidence(hints, minConfidence) {
...
//# sourceMappingURL=source-hint-engine.js.map