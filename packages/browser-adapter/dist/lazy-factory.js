 ...
 */
function getAdapterLoader(config) {
 ...
 */
export async function detectAdapterLazy(options = {}) {
    ...
    if (allowFallback) {
        ...
        const vanillaLoader = getAdapterLoader(LAZY_ADAPTERS.find((a) => a.name === 'vanilla'));
        try {
 ...
 */
export async function getAdapterLazy(options = {}) {
 ...
 */
export async function createAdapterLazy(options = {}) {
    return getAdapterLazy(options);
}
/**
 * Get adapter by name (lazy loading)
 */
export async function getAdapterByNameLazy(name) {
 ...
 */
export function clearAdapterCache(name) {
    if (name) {
 ...
 */
export function getLoadedAdapters() {
    const loaded = [];
