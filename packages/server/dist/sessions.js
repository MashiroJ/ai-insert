...
import { SELECTION_TTL_MS, MAX_MESSAGES_PER_SESSION } from './state.js';
...
export function upsertSessionFromSelection(selection, state, incomingTargets, mode, diagnostics) {
    const now = Date.now();
...
export const VALID_STYLE_SOURCE_HINT_KINDS = [
    'vue-sfc-style-rule',
    'style-rule',
    'template-class',
    'inline-style',
...
export function normalizeStyleSourceHints(value) {
    ...
    return value
        ...
        .filter((item) => VALID_STYLE_SOURCE_HINT_KINDS.includes(item.kind))
        .map((item) => ({
