...
export const VALID_STYLE_SOURCE_HINT_KINDS: UiInspectStyleSourceHintKind[] = [
  'vue-sfc-style-rule',
  'style-rule',
  'template-class',
  'inline-style',
...
export function normalizeStyleSourceHints(value: unknown): UiInspectStyleSourceHint[] | undefined {
  ...
  return value
    ...
    .filter((item) => VALID_STYLE_SOURCE_HINT_KINDS.includes(item.kind as UiInspectStyleSourceHintKind))
    .map((item) => ({
      id: stringOr(item.id, `hint-${Date.now()}`),
      targetId: stringOr(item.targetId, ''),
      kind: item.kind as UiInspectStyleSourceHintKind,
...
export function createMessage(sessionId: string, role: UiInspectMessageRole, content: string, selectionId: string | null): UiInspectMessage {
  return {
...
export function appendMessage(sessionId: string, role: UiInspectMessageRole, content: string, selectionId: string | null, state: ServerState): UiInspectMessage {
...
export function appendAssistantMessage(sessionId: string, content: string, selectionId: string | null, state: ServerState): UiInspectMessage {
  const session = state.sessions.get(sessionId);
  if (!session) throw new Error(`session not found: ${sessionId}`);
  ...
  return appendMessage(sessionId, 'assistant', content, selectionId, state);
}
