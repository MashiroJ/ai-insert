 ...
 */
export function resolveCssDebugBoundary(element: HTMLElement): ScopeGuard | null {
 ...
 */
export function clampRectToBoundary(rect: Rect, boundary: Rect): {
  clamped: boolean;
  result: Rect;
 ...
 */
export function clampMoveInteraction(
  elementRect: Rect,
  boundary: Rect,
  dx: number,
  dy: number,
 ...
 */
export function clampResizeInteraction(
  elementRect: Rect,
  boundary: Rect,
  handle: string,
  dx: number,
...
function findComponentRoot(element: HTMLElement): HTMLElement | null {
...
function findStableContainer(element: HTMLElement): HTMLElement | null {
  let current: HTMLElement | null = element.parentElement;
  while (current && current !== document.body) {
    const styles = window.getComputedStyle(current);
    const hasExplicitSize =
      (styles.width && styles.width !== 'auto' && styles.width !== '') ||
...
function getSafeRect(element: HTMLElement): Rect | null {
  try {
...
function isRectValid(rect: Rect | null): rect is Rect {
