...
import { TOGGLE_ID, DIANA_SIZE, DIANA_SPRITE_URL, } from './constants.js';
import { readDianaPosition, applyDianaPosition, } from './position.js';
import { setupDragListeners } from './drag.js';
import { setDianaState, updateDianaDirection } from './state.js';
/**
 * Create Diana component
 */
export function createDiana(options = {}) {
    const { id = TOGGLE_ID, spriteUrl = DIANA_SPRITE_URL, initialState = 'idle', onDragStart, onDragEnd, onClick, onHover, onMenuOpen, } = options;
 ...
 * Get Diana element
 */
export function getDianaElement() {
    return document.getElementById(TOGGLE_ID);
}
/**
 * Update Diana state
 */
export function updateDianaState(state, temporary) {
