...
function summarizeTargets(targets, cssDebug) {
    ...
    return targets.map((target, index) => {
        ...
        if (debugTarget) {
            ...
            const primaryInteraction = debugTarget.primaryInteraction;
            const interactions = debugTarget.interactions;
            ...
            if (reorderIx) {
                const reorder = reorderIx.reorder;
                if (reorder) {
            ...
            if (groupScaleIx) {
                const gs = groupScaleIx.groupScale;
                if (gs) {
...
function summarizeCssDebug(cssDebug) {
    ...
    if (Array.isArray(targets) && targets.length > 0) {
        ...
        for (const t of targets.slice(0, 5)) {
            ...
            const primaryInteraction = t.primaryInteraction;
            ...
            const groupScaleIx = allInteractions.find((ix) => ix.type === 'group-scale');
            if (groupScaleIx) {
                const groupScale = groupScaleIx.groupScale;
