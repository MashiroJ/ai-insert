...
const KIND_PRIORITY = ['next', 'rsbuild', 'rspack', 'vite', 'webpack'];
...
export function detectProject(project) {
    ...
    if (result.kind === 'next') {
        ...
        result.next = { router };
        if (router === 'unknown')
            result.warnings.push('Next project detected, but no app/ or pages/ router entry files were found');
    }
    return result;
}
function detectConfigFiles(project) {
    const matches = {};
...
function readPackageJson(project, warnings) {
    try {
...
function chooseProjectKind(configMatches, dependencies) {
    for (const kind of KIND_PRIORITY) {
        if (configMatches[kind].length > 0)
            return kind;
    }
    for (const kind of KIND_PRIORITY) {
        if (DEPENDENCY_CANDIDATES[kind].some((name) => dependencies[name]))
            return kind;
    }
    return 'unknown';
}
function detectNextRouter(project, matchedFiles) {
    ...
    if (app)
        matchedFiles.push(app);
    if (pages)
        matchedFiles.push(pages);
    if (app && pages)
        return 'both';
    if (app)
        return 'app';
...
function findRouterRoot(project, roots, fileNames) {
    for (const root of roots) {
        const directory = join(project, root);
        if (!isDirectory(directory))
            continue;
        if (hasAnyRouterFile(directory, fileNames))
            return directory;
    }
    return null;
}
function hasAnyRouterFile(directory, fileNames) {
    for (const fileName of fileNames) {
        for (const extension of ROUTER_EXTENSIONS) {
            if (existsSync(join(directory, `${fileName}.${extension}`)))
                return true;
        }
    }
    return false;
