...
import { delay } from '@ui-inspect/server';
...
export async function ensureProjectDevServer({ project, openBrowser = false, timeoutMs = 10000, }) {
    ...
    if (existing) {
        ...
        result.url = existing;
        if (openBrowser)
            result.opened = openUrl(existing);
        return result;
    }
...
async function findViteServer() {
    for (const port of VITE_PORTS) {
        if (await isVitePort(port))
            return `http://localhost:${port}`;
    }
    return null;
}
async function isVitePort(port) {
    try {
...
function packageManager(project) {
    ...
    if (existsSync(join(project, 'pnpm-lock.yaml')))
        return 'pnpm';
    if (existsSync(join(project, 'bun.lockb')) || existsSync(join(project, 'bun.lock')))
        return 'bun';
    return commandExists('yarn') ? 'yarn' : 'npm';
}
function packageManagerEnv(command) {
    return command === 'yarn'
