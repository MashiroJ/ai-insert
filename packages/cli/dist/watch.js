...
export async function runWatch(options = {}) {
    ...
    if (!acquireWatcherLock(project, daemonUrl))
        return;
    ...
    if (agent === 'codex')
        process.stdout.write(`codex transport: ${codexTransportMode()}\n`);
    ...
    while (true) {
        try {
            ...
            for (const item of messages) {
                if (running)
                    continue;
                if (!claimMessage(project, item.message.id))
                    continue;
                try {
                    running = true;
...
function collectPendingUserMessages(sessions) {
    const pending = [];
...
function markExistingUserMessagesSeen(sessions) {
...
async function runAgent(input) {
    ...
    const appendLog = (text) => appendFileSync(logFile, text);
...
function spawnAgent(agent, project) {
    if (agent === 'claude') {
        return spawn('claude', [
            '-p',
...
function codexTransportMode() {
    return process.env.AI_INSPECT_CODEX_TRANSPORT === 'default' ? 'default' : 'http';
}
function codexTransportArgs() {
    if (codexTransportMode() === 'default')
        return [];
    return [
...
function createAgentParser(agent, onEvent) {
    let buffer = '';
    ...
    const handleLine = (line) => {
        ...
        if (event?.type === 'text_delta' && agent === 'claude')
            claudeTextEmitted = true;
        if (event)
            void onEvent(event);
    };
    return {
...
function parseClaudeEvent(value, textAlreadyEmitted) {
    if (!isRecord(value))
        return null;
    ...
    if (value.type === 'result' && typeof value.result === 'string' && !textAlreadyEmitted)
        return { type: 'text_delta', delta: value.result };
    if (value.type === 'error' && typeof value.message === 'string')
        return { type: 'error', text: value.message };
    return null;
}
function buildPrompt(agent, session, message) {
...
function aiInspectDir(project) {
    const dir = join(project, '.ai-insert');
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    return dir;
}
function claimMessage(project, messageId) {
...
function acquireWatcherLock(project, daemonUrl) {
...
function isTransientAgentError(text) {
