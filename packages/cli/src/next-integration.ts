...
export function inspectNextIntegration(project: string, router: NextRouterKind): NextIntegrationInspection {
  ...
  const missing: NextIntegrationMissing[] = [];

  if (!hasPackage) missing.push(NEXT_PACKAGE);
  if (!hasScript) missing.push('UiInspectScript');
  if (!hasRoute) missing.push('diana-route');
...
function routerFamilies(router: NextRouterKind): RouterFamily[] {
  if (router === 'app') return ['app'];
  if (router === 'pages') return ['pages'];
  return ['app', 'pages'];
}

function scriptCandidates(families: RouterFamily[]): string[] {
  return families.flatMap((family) => family === 'app' ? APP_SCRIPT_CANDIDATES : PAGES_SCRIPT_CANDIDATES);
}

function routeCandidates(families: RouterFamily[]): string[] {
  return families.flatMap((family) => family === 'app' ? APP_ROUTE_CANDIDATES : PAGES_ROUTE_CANDIDATES);
}

function existingFiles(project: string, candidates: string[]): string[] {
  return candidates.map((name) => join(project, name)).filter((file) => existsSync(file));
}

function readPackageJson(project: string): PackageJsonLike | null {
  try {
...
function snippetsForRouter(router: NextRouterKind): NextIntegrationSnippets {
  if (router === 'app') return { appRouter: APP_ROUTER_SNIPPETS };
  if (router === 'pages') return { pagesRouter: PAGES_ROUTER_SNIPPETS };
  return {
...
const APP_ROUTER_SNIPPETS: NextIntegrationSnippetSet = {
...
const PAGES_ROUTER_SNIPPETS: NextIntegrationSnippetSet = {
  script: `import { UiInspectScript } from '@ui-inspect/next';
