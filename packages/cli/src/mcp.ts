  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name === 'get_frontend_source') {
        ...
        if (!selection.active || !selection.selection) throw new Error('No active ai-inspect selection.');
        const context = contextLines(args.context);
...
function latestUserMessage(
  payload: { sessions: AiInspectSession[] },
  sinceTimestamp: number,
): { session: AiInspectSession; message: AiInspectMessage } | null {
  let latest: { session: AiInspectSession; message: AiInspectMessage } | null = null;
...
async function readSourceIfAvailable(selection: AiInspectSelection | null, context: number) {
