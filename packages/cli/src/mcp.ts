...
function summarizeTargets(targets: NonNullable<UiInspectSession['targets']>, cssDebug?: unknown): string {
  if (!targets.length) return 'No targets.';
  const debugTargets = cssDebug && typeof cssDebug === 'object'
    ? (cssDebug as Record<string, unknown>).targets as Array<Record<string, unknown>> | undefined
    : undefined;
  return targets.map((target, index) => {
    ...
    const debugTarget = debugTargets?.find(
      (t) => t.id === target.id || (t.selection as Record<string, unknown>)?.id === selection.id,
    );
    if (debugTarget) {
      const changed = debugTarget.changedStyles as Record<string, { originalValue?: string; previewValue?: string }> | undefined;
      if (changed && typeof changed === 'object') {
        const entries = Object.entries(changed)
          .map(([prop, val]) => `${prop} ${val.originalValue ?? 'auto'} -> ${val.previewValue ?? 'auto'}`)
          .join(', ');
        if (entries) line += ` | changed: ${entries}`;
      }
      const styleHints = debugTarget.styleSourceHints as Array<Record<string, unknown>> | undefined;
      ...
      const layoutHints = debugTarget.layoutHints as Array<Record<string, unknown>> | undefined;
...
function summarizeCssDebug(cssDebug: unknown): string {
  ...
  if (Array.isArray(targets) && targets.length > 0) {
    ...
    for (const t of targets.slice(0, 5) as Record<string, unknown>[]) {
      ...
      const changed = t.changedStyles as Record<string, { originalValue?: unknown; previewValue?: unknown }> | undefined;
      const props = changed
      ...
      const styleHints = t.styleSourceHints as Array<Record<string, unknown>> | undefined;
      if (Array.isArray(styleHints) && styleHints.length > 0) {
        const best = styleHints[0];
        const hintLine = best.line ? `:${best.line}` : '';
      ...
      const layoutHints = t.layoutHints as Array<Record<string, unknown>> | undefined;
      if (Array.isArray(layoutHints) && layoutHints.length > 0) {
      ...
      const specWarnings = t.specificityWarnings as Array<Record<string, unknown>> | undefined;
  ...
  // Scope guard summary
  const scopeGuard = payload.scopeGuard as Record<string, unknown> | undefined;
  ...
  if (Array.isArray(targets)) {
    for (const t of targets.slice(0, 5) as Record<string, unknown>[]) {
      const tScope = t.scopeGuard as Record<string, unknown> | undefined;
...
function compactCssDebug(cssDebug: unknown, topLevelSelectionId?: string): unknown {
  ...
  if (Array.isArray(payload.targets)) {
    result.targets = (payload.targets as Record<string, unknown>[]).map((t) => {
      const targetSel = (t.selection ?? null) as UiInspectSelection | null;
...
export function compactFrontendRequestResult(result: unknown): Record<string, unknown> {
  ...
  return {
    ...
    cssDebug: compactCssDebug(cssDebug, (r.selection as UiInspectSelection | null)?.id),
    source,
  };
}
