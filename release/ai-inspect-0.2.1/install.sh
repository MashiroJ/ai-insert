...
install_vite_plugin() {
  ...
  if [[ "$package_manager" == "pnpm" ]]; then
    if [[ "$SOURCE" == "local" ]]; then
      (cd "$PROJECT_DIR" && pnpm add -D "$DIR"/mashiro39-ai-inspect-protocol-0.2.1.tgz "$DIR"/mashiro39-ai-inspect-vite-plugin-0.2.1.tgz)
    else
      (cd "$PROJECT_DIR" && pnpm add -D @mashiro39/ai-inspect-vite-plugin@0.2.1)
    fi
  elif [[ "$package_manager" == "yarn" ]]; then
    if [[ "$SOURCE" == "local" ]]; then
      (cd "$PROJECT_DIR" && COREPACK_ENABLE_STRICT=0 yarn add -D "$DIR"/mashiro39-ai-inspect-protocol-0.2.1.tgz "$DIR"/mashiro39-ai-inspect-vite-plugin-0.2.1.tgz)
    else
      (cd "$PROJECT_DIR" && COREPACK_ENABLE_STRICT=0 yarn add -D @mashiro39/ai-inspect-vite-plugin@0.2.1 --registry https://registry.npmjs.org)
    fi
  else
    if [[ "$SOURCE" == "local" ]]; then
      npm install --save-dev --prefix "$PROJECT_DIR" "$DIR"/mashiro39-ai-inspect-protocol-0.2.1.tgz "$DIR"/mashiro39-ai-inspect-vite-plugin-0.2.1.tgz
    else
      npm install --save-dev --prefix "$PROJECT_DIR" @mashiro39/ai-inspect-vite-plugin@0.2.1
    fi
  fi
