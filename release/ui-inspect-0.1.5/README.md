...
```bash
npm install -g ./ui-inspect-protocol-0.1.5.tgz ./ui-inspect-server-0.1.5.tgz ./ui-inspect-cli-0.1.5.tgz
npm install -D ./ui-inspect-protocol-0.1.5.tgz ./ui-inspect-vite-plugin-0.1.5.tgz
```
...
- Diana 会跟随 AI task 状态切换动作。
- 点击 Diana 可选择定位源码、单点修改或批量标注。
- 定位源码只尝试打开源码并给出结果提示，不创建 AI task。
- 单点修改只保留一个修改需求输入框；批量标注支持连续点选，点完成后统一整理备注和整体需求。
- 浏览器面板支持多选元素。
- 每个元素可以填写单独备注。
