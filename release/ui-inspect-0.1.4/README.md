...
```bash
npm install -g ./ui-inspect-protocol-0.1.4.tgz ./ui-inspect-server-0.1.4.tgz ./ui-inspect-cli-0.1.4.tgz
npm install -D ./ui-inspect-protocol-0.1.4.tgz ./ui-inspect-vite-plugin-0.1.4.tgz
```
...
- 定位源码不会创建 AI task；单点修改和批量标注进入明确的任务草稿。
- 浏览器面板支持多选元素。
- 每个元素可以填写单独备注。
