...
- 定位源码：点选元素后直接打开对应源码文件，不创建 AI 任务。
- 单点修改：选择一个元素，填写修改要求，发送给 AI。
- 批量标注：连续选择多个元素，每个目标可写备注，再统一发送给 AI。
- 历史会话：查看最近的调试会话。
...
- 浏览器 UI：负责点选元素、打开源码、批量标注、输入需求、查看历史和展示任务状态。
