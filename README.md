...
```bash
./release/ai-inspect-0.1.0/install.sh --mode mcp
./release/ai-inspect-0.1.0/install.sh --mode vite --project /path/to/vite-vue-project
./release/ai-inspect-0.1.0/install.sh --mode all --project /path/to/vite-vue-project
```

安装脚本默认使用公网 npm 包。只有显式传入 `--source local` 时，才会使用 release 目录里的本地 tgz：

```bash
./release/ai-inspect-0.1.0/install.sh --source local --mode all --project /path/to/vite-vue-project
```
