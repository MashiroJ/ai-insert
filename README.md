...
其中 `@ui-inspect/vite-plugin` 仍是验证最充分的推荐入口；`@ui-inspect/webpack-plugin`、`@ui-inspect/rspack-plugin`、`@ui-inspect/rsbuild-plugin` 适合非 Vite 项目接入和适配验证；`@ui-inspect/next` 用于 Next.js App Router / Pages Router 项目。
