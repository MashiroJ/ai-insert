...
1. 点击 Diana。
2. 选择 `定位源码`、`单点修改` 或 `批量标注`。
3. `定位源码`：点选元素后直接打开源码文件，不创建 AI task。
4. `单点修改`：点选一个元素，填写修改要求后发送给 AI。
5. `批量标注`：连续点选多个目标，点 `完成标注` 后再给每个目标写备注，并可填写整体需求发送给 AI。
6. AI 会通过 `wait_for_frontend_request` 收到 task、targets、每个元素备注和源码上下文，然后继续改代码。
