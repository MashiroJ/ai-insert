...
```bash
pnpm add -D @mashiro39/ui-inspect-vite-plugin@latest
```

yarn：

```bash
yarn add -D @mashiro39/ui-inspect-vite-plugin@latest
```
...
2. 安装 `@mashiro39/ui-inspect-vite-plugin@latest`。
...
```bash
npx -y @mashiro39/ui-inspect-cli@latest mcp
```
...
- MCP 配置是否使用了 `npx -y @mashiro39/ui-inspect-cli@latest mcp`。
